...
@end
@implementation GHDBModel


#pragma mark - override method
+ (void)initialize
{
    if (self != [GHDBModel self]) {
        [self createTable];
    }
}
...
+ (NSDictionary *)getPropertys
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        /*
         ...
         后来项目中新增了实体类型和Bool类型，对应SQLDATA和SQLBOOL,如果存储自定义model（比如：JZGProgramDataModel、JZGTaskTypeModel），需要加一个判断条件
         */
        ...
        } else if ([propertyType hasPrefix:@"T@\"JZGProgramDataModel\""]
                   ||[propertyType hasPrefix:@"T@\"JZGTaskTypeModel\""]
...
{
    __block BOOL res = NO;
    GHDBHelper *jzgDB = [GHDBHelper shareInstance];
    [jzgDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    ...
    GHDBHelper *jzgDB = [GHDBHelper shareInstance];
    [jzgDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
- (BOOL)save
{
    ...
    for (int i = 0; i < self.columeNames.count; i++) {
        ...
        NSString *protype = [self.columeTypes objectAtIndex:i];
        ...
        id value;
        if ([protype isEqualToString:SQLDATA]) {
            value = [NSKeyedArchiver archivedDataWithRootObject:[self valueForKey:proname]];
        }else{
            value = [self valueForKey:proname];
        }
        [insertValues addObject:value ? value: @""];
    }
...
{
    //判断是否是JKBaseModel的子类
    for (GHDBModel *model in array) {
        if (![model isKindOfClass:[GHDBModel class]]) {
            return NO;
        }
    }
    ...
    [jzgDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (GHDBModel *model in array) {
            ...
            for (int i = 0; i < model.columeNames.count; i++) {
                ...
                NSString *protype = [model.columeTypes objectAtIndex:i];
                if ([proname isEqualToString:primaryId]) {
                    continue;
                }
                [keyString appendFormat:@"%@,", proname];
                ...
                id value;
                if ([protype isEqualToString:SQLDATA]) {
                    value = [NSKeyedArchiver archivedDataWithRootObject:[model valueForKey:proname]];
                }else{
...
- (BOOL)update
{
    ...
    [jzgDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        for (int i = 0; i < self.columeNames.count; i++) {
            ...
            [updateValues addObject:value ? value: @""];
        }
...
+ (NSArray *)findAll
{
    ...
    [jzgDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            GHDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
        ...
        while ([resultSet next]) {
            ...
            for (int i=0; i< model.columeNames.count; i++) {
                ...
                if ([columeType isEqualToString:SQLTEXT]) {
                    ...
                    id objTemp = [NSKeyedUnarchiver unarchiveObjectWithData:[resultSet dataForColumn:columeName]];
                    [model setValue:objTemp forKey:columeName];
                } else if ([columeType isEqualToString:SQLBOOL]) {
                    [model setValue:[NSNumber numberWithBool:[resultSet boolForColumn:columeName]] forKey:columeName];
                } else{
                    [model setValue:[NSNumber numberWithLongLong:[resultSet longLongIntForColumn:columeName]] forKey:columeName];
...
+ (NSInteger)findCountBySql:(NSString *)sqlString{
    __block NSInteger totalCount = 0;
    GHDBHelper *jzgDB = [GHDBHelper shareInstance];
    [jzgDB.dbQueue inDatabase:^(FMDatabase *db) {
        totalCount = [db intForQuery:sqlString];
    }];
    return totalCount;
}
